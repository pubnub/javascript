...
import {
  ...
  getfile,
  postfile,
} from '../networking/modules/web-node';
...
function stringifyBufferKeys(obj) {
  ...
  Object.keys(obj).forEach((key) => {
    ...
    if (Array.isArray(key) || (keyIsString && key.indexOf(',') >= 0)) {
      ...
      stringifiedKey = String.fromCharCode(
        keyIsString ? parseInt(key, 10) : 10
      );
    }

    normalizedObject[stringifiedKey] = isObject(value)
      ? stringifyBufferKeys(value)
      : value;
  });
