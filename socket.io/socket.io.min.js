...
function N(a){return M(encodeURIComponent(a).split(""),function(a){return 0>"-_.!~*'()".indexOf(a)?a:"%"+a.charCodeAt(0).toString(16).toUpperCase()}).join("")}function ma(a){var c=[];K(a,function(a,d){d.g&&c.push(a)});return c.sort()}function O(){setTimeout(function(){u||(u=1,K(ca,function(a){a()}))},B)}
...
set:s()},I=a.uuid||H&&H.get(i+"uuid")||"",z={LEAVE:function(a,b){var c={uuid:I},d=G(k),e=D();0<a.indexOf(x)||("0"!=e&&(c.callback=e),A({h:b||t,timeout:2E3,a:e,data:c,url:[d,"v2","presence","sub_key",i,"channel",N(a),"leave"]}))},history:function(a,b){var b=a.callback||b,c=a.count||a.limit||100,d=a.reverse||"false",e=a.error||s(),f=a.channel,h=a.start,j=a.end,g={},k=D();if(!f)return p("Missing Channel");if(!b)return p("Missing Callback");if(!i)return p("Missing Subscribe Key");g.stringtoken="true";
g.count=c;g.reverse=d;k&&(g.callback=k);h&&(g.start=h);j&&(g.end=j);A({a:k,data:g,c:function(a){b(a)},b:e,url:[m,"v2","history","sub-key",i,"channel",N(f)]})},replay:function(a){var b=b||a.callback||s(),c=a.source,d=a.destination,e=a.stop,f=a.start,h=a.end,g=a.reverse,a=a.limit,k=D(),l={};if(!c)return p("Missing Source Channel");if(!d)return p("Missing Destination Channel");if(!j)return p("Missing Publish Key");if(!i)return p("Missing Subscribe Key");"0"!=k&&(l.callback=k);e&&(l.stop="all");g&&(l.reverse=
"true");f&&(l.start=f);h&&(l.end=h);a&&(l.count=a);A({a:k,c:function(a){b(a)},b:function(){b([0,"Disconnected"])},url:[m,"v1","replay",j,i,c,d],data:l})},time:function(a){var b=D();A({a:b,timeout:5*B,url:[m,"time",b],c:function(b){a(b[0])},b:function(){a(0)}})},publish:function(a,b){var b=b||a.callback||s(),c=a.message,d=a.channel,e=D();if(!c)return p("Missing Message");if(!d)return p("Missing Channel");if(!j)return p("Missing Publish Key");if(!i)return p("Missing Subscribe Key");c=JSON.stringify(c);
d=[m,"publish",j,i,0,N(d),e,N(c)];r.push({a:e,timeout:5*B,url:d,data:{uuid:I},c:function(a){b(a);l(1)},b:function(){b([0,"Failed",c]);l(1)}});l()},unsubscribe:function(a){a=a.channel;L=0;aa=1;a=M((a.join?a.join(","):""+a).split(","),function(a){return a+","+a+x}).join(",");K(a.split(","),function(a){u&&z.LEAVE(a,0);y[a]=0});u&&c()},subscribe:function(a,b){function d(a){a?setTimeout(f,B):(m=G(k,1),h=G(k,1),setTimeout(function(){z.time(d)},B));e(function(b){if(a&&b.d)return b.d=0,b.m(b.name);!a&&!b.d&&
(b.d=1,b.l(b.name))})}function f(){var a=D(),b=ma(y).join(",");b&&(v=A({timeout:da,a:a,b:function(){z.time(d)},data:{uuid:I},url:[h,"subscribe",i,N(b),a,L],c:function(a){if(!a)return setTimeout(f,sa);e(function(a){a.f||(a.f=1,a.k(a.name))});L=!L&&aa&&H.get(i)||a[1];ra&&(Timetoken=1E4,ra=0);H.set(i,a[1]);var b,c=(2<a.length?a[2]:"").split(",");b=function(){var a=c.shift()||"";return[(y[a]||{}).a||U,(a||V).split(x)[0]]};K(a[0],function(c){var d=b();if(y[d[1]].g)d[0](c,a,d[1])});setTimeout(f,sa)}}))}
var g=a.channel,b=(b=b||a.callback)||a.message,j=a.connect||s(),l=a.reconnect||s(),t=a.disconnect||s(),r=a.presence||0,w=a.noheresync||0,ra=a.backfill||0,da=a.timeout||F,sa=a.windowing||P;a.restore&&(aa=1);L=0;if(!g)return p("Missing Channel");if(!b)return p("Missing Callback");if(!i)return p("Missing Subscribe Key");K((g.join?g.join(","):""+g).split(","),function(a){var c=y[a]||{};y[V=a]={name:a,f:c.f,d:c.d,g:1,a:U=b,k:j,l:t,m:l};r&&(z.subscribe({channel:a+x,callback:r}),!c.g&&!w&&z.here_now({channel:a,
callback:function(b){K("uuids"in b?b.uuids:[],function(c){r({action:"join",uuid:c,timestamp:E(),occupancy:b.occupancy||1},b,a)})}}))});c=function(){v&&v(1);clearTimeout(na);na=setTimeout(f,100)};if(!u)return ca.push(c);c()},here_now:function(a,b){var b=a.callback||b,c=a.error||s(),d=a.channel,e=D(),f=n;if(!d)return p("Missing Channel");if(!b)return p("Missing Callback");if(!i)return p("Missing Subscribe Key");"0"!=e&&(f={},f.callback=e);A({a:e,data:f,c:function(a){b(a)},b:c,url:[m,"v2","presence",
"sub_key",i,"channel",N(d)]})},xdr:A,ready:O,db:H,uuid:ha,each:K,map:M,grep:ka,supplant:la,now:E,unique:C,updater:ja};I||(I=z.uuid());H.set(i+"uuid",I);setTimeout(d,B);setTimeout(b,g);f=z;f.css=ta;f.$=Q;f.create=ua;f.bind=R;f.head=qa;f.search=pa;f.attr=S;f.events=Y;f.init=Da;if(a.notest)return f;R("beforeunload",window,function(){each_channel(function(a){f.LEAVE(a.name,1)});return!0});R("offline",window,f._reset_offline);R("offline",document,f._reset_offline);return f};R("load",window,function(){setTimeout(O,
...
Ea.prototype.send=function(a){var c=this;c.e.publish({channel:c.e.j.channel,message:a,callback:function(a){c.onsend({data:a})}})};
})();
