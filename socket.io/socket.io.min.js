...
var a={id:U.id++,send:t(),abort:function(){a.id={}},open:function(c,b){U[a.id]=a;Ba.get(a.id,b)}};return a},za="async",xa="PubNub-JS-Web/3.5.1",T=-1==navigator.userAgent.indexOf("MSIE 6");window.console||(window.console=window.console||{});console.log||(console.log=console.error=(window.opera||{}).postError||t());var Ca,V=window.localStorage;Ca={get:function(a){try{return V?V.getItem(a):-1==document.cookie.indexOf(a)?n:((document.cookie||"").match(RegExp(a+"=([^;]+)"))||[])[1]||n}catch(c){}},set:function(a,
...
z));s(function(b){if(a&&b.d)return b.d=0,b.m(b.name);!a&&!b.d&&(b.d=1,b.l(b.name))})}function i(){var a=F(),d=oa(A).join(",");d&&(b(),J=B({timeout:Fa,a:a,b:function(){J=n;y.time(e)},data:{uuid:C,auth:m},url:[O,"subscribe",j,N(d),a,K],c:function(a){J=n;if(!a||"error"in a&&!a.error)return u(a),setTimeout(c,ga);K=!K&&ca&&L.get(j)||a[1];s(function(a){a.f||(a.f=1,a.k(a.name))});ta&&(K=1E4,ta=0);L.set(j,a[1]);var b,d=(2<a.length?a[2]:P(A,function(b){return P(Array(a[0].length).join(",").split(","),function(){return b})}).join(",")).split(",");
b=function(){var a=d.shift()||X;return[(A[a]||{}).a||Y,a.split(w)[0]]};M(a[0],function(c){var d=b();d[0](c,a,d[1])});setTimeout(i,ga)}}))}var g=a.channel,d=(d=d||a.callback)||a.message,k=a.connect||t(),l=a.reconnect||t(),q=a.disconnect||t(),u=a.error||t(),D=a.presence||0,fa=a.noheresync||0,ta=a.backfill||0,Ga=a.timetoken||0,Fa=a.timeout||h,ga=a.windowing||H;ca=a.restore;K=Ga;if(!g)return p("Missing Channel");if(!d)return p("Missing Callback");if(!j)return p("Missing Subscribe Key");M((g.join?g.join(","):
""+g).split(","),function(a){var b=A[a]||{};A[X=a]={name:a,f:b.f,d:b.d,j:1,a:Y=d,k:k,l:q,m:l};D&&(y.subscribe({channel:a+w,callback:D}),!b.j&&!fa&&y.here_now({channel:a,callback:function(b){M("uuids"in b?b.uuids:[],function(c){D({action:"join",uuid:c,timestamp:G(),occupancy:b.occupancy||1},b,a)})}}))});c=function(){b();setTimeout(i,ga)};if(!v)return ba.push(c);c()},here_now:function(a,b){var b=a.callback||b,c=a.error||t(),d=a.channel,e=F(),f={uuid:C,auth:m};if(!d)return p("Missing Channel");if(!b)return p("Missing Callback");
if(!j)return p("Missing Subscribe Key");"0"!=e&&(f.callback=e);B({a:e,data:f,c:function(a){b(a)},b:c,url:[x,"v2","presence","sub_key",j,"channel",N(d)]})},xdr:B,ready:pa,db:L,uuid:ja,map:P,each:M,"each-channel":s,grep:ma,offline:function(){b(1)},supplant:na,now:G,unique:E,updater:la};C||(C=y.uuid());L.set(j+"uuid",C);setTimeout(e,z);setTimeout(d,i);y.time(t());g=y;g.css=ua;g.$=Q;g.create=va;g.bind=R;g.head=sa;g.search=ra;g.attr=S;g.events=W;g.init=Da;R("beforeunload",window,function(){g["each-channel"](function(a){g.LEAVE(a.name,
