...
b){function f(a){a?setTimeout(d,J):(K=na(ma,1),qb=na(ma,1),setTimeout(function(){A.time(f)},J));B(function(b){if(a&&b.d)return b.d=0,b.j(b.name);!a&&!b.d&&(b.d=1,b.i(b.name))});w(function(b){if(a&&b.d)return b.d=0,b.j(b.name);!a&&!b.d&&(b.d=1,b.i(b.name))})}function h(){var a=F(),b=va(C).join(","),g=wa(N).join(",");if(b||g){b||(b=",");c();var m=D({uuid:H,auth:p});g&&(m["channel-group"]=g);2<JSON.stringify(Q).length&&(m.state=JSON.stringify(Q));V&&(m.heartbeat=V);e();Ca=E({timeout:ba,a:a,b:function(a){a&&
