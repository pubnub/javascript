...
class RequestSignature {
    ...
    signature(req) {
        ...
        if (method === transport_request_1.TransportMethod.POST || method === transport_request_1.TransportMethod.PATCH || method === transport_request_1.TransportMethod.PUT) {
            const body = req.body;
            let payload;
            ...
            const retryableRequest = new Promise((resolve, reject) => {
                const trySendRequest = () => {
                    ...
                    const responseHandler = (res, error) => {
                        const retriableError = !error || error.category !== categories_1.default.PNCancelledCategory;
