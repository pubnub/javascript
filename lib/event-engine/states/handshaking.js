...
exports.HandshakingState.on(events_1.disconnect.type, (context, event) => {
    ...
    if (!event.payload.isOffline)
        return handshake_stopped_1.HandshakeStoppedState.with({ channels: context.channels, groups: context.groups, cursor: context.cursor });
    else {
        ...
        return handshake_failed_1.HandshakeFailedState.with({ channels: context.channels, groups: context.groups, cursor: context.cursor, reason: errorReason }, [
            (0, effects_1.emitStatus)({
