...
import { disconnect, joined, left, leftAll, reconnect } from '../events';
import { leave } from '../effects';
...
export const HeartbeatFailedState = new State('HEARTBEAT_FAILED');
...
HeartbeatFailedState.on(reconnect.type, (context, _) => HeartbeatingState.with({
    channels: context.channels,
    groups: context.groups,
}));
HeartbeatFailedState.on(disconnect.type, (context, _) => HeartbeatStoppedState.with({
    channels: context.channels,
    groups: context.groups,
