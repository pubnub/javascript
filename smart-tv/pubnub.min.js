...
set:function(a,d){try{if(Rb)return Rb.setItem(a,d)&&0;Vb&&Ub(a,d);Qb[a]=d}catch(b){Qb[a]=d}}};var Yb={list:{},unbind:function(a){Yb.list[a]=[]},bind:function(a,d){(Yb.list[a]=Yb.list[a]||[]).push(d)},fire:function(a,d){M(Yb.list[a]||[],function(a){a(d)})}},Zb=tb("pubnub")||0,ac=function(a){function d(){}function b(a,b){function c(b){b&&(db=H()-(b/1E4+(H()-d)/2),a&&a(db))}var d=H();b&&c(b)||C.time(c)}function c(a,b){Oa&&Oa(a,b);Oa=y;clearTimeout($);clearTimeout(R)}function e(){bc&&C.time(function(a){b(A(),
a);a||c(1,{error:"Heartbeat failed to connect to Pubnub Servers.Please check your network settings."});R&&clearTimeout(R);R=setTimeout(e,Db)})}function p(){cc()||c(1,{error:"Offline. Please check your network settings. "});$&&clearTimeout($);$=setTimeout(p,E)}function q(a,b,c,d){var b=a.callback||b,e=a.error||t,f=J(),d=d||{};d.auth||(d.auth=a.auth_key||K);a=[L,"v1","channel-registration","sub-key",w];a.push.apply(a,c);f&&(d.callback=f);I({a:f,data:B(d),c:function(a){s(a,b,e)},b:function(a){j(a,e)},
url:a})}function j(a,b){if("object"==typeof a&&a.error){var c={};a.message&&(c.message=a.message);a.payload&&(c.payload=a.payload);b&&b(c)}else b&&b(a)}function s(a,b,c){if("object"==typeof a){if(a.error){b={};a.message&&(b.message=a.message);a.payload&&(b.payload=a.payload);c&&c(b);return}if(a.payload){a.next_page?b&&b(a.payload,a.next_page):b&&b(a.payload);return}}b&&b(a)}function D(a){var b=0;M(ya(F),function(c){if(c=F[c])b++,(a||A())(c)});return b}function u(a){var b=0;M(za(N),function(c){if(c=
N[c])b++,(a||A())(c)})}function r(a){if(dc){if(!aa.length)return}else{a&&(aa.n=0);if(aa.n||!aa.length)return;aa.n=1}I(aa.shift())}function f(){!eb&&h()}function h(){clearTimeout(Pa);!O||500<=O||1>O||!ya(F,v).length&&!za(N,v).length?eb=z:(eb=v,C.presence_heartbeat({callback:function(){Pa=setTimeout(h,O*E)},error:function(a){t&&t("Presence Heartbeat unable to reach Pubnub servers."+JSON.stringify(a));Pa=setTimeout(h,O*E)}}))}function m(a,b){return Qa.decrypt(a,b||V)||Qa.decrypt(a,V)||a}function g(a,
...
Nb=a["compatible_3.5"]||z,I=a.xdr,P=a.params||{},t=a.error||A(),cc=a._is_online||function(){return 1},J=a.jsonp_cb||function(){return 0},ia=a.db||{get:A(),set:A()},V=a.cipher_key,G=a.uuid||!a.unique_uuid&&ia&&ia.get(w+"uuid")||"",$,R,Qa=a.crypto_obj||{encrypt:function(a){return a},decrypt:function(a){return a}},C={LEAVE:function(a,b,c,d,e){var c={uuid:G,auth:c||K},f=pa(oa),d=d||A(),h=e||A(),e=J();if(0<a.indexOf(ga))return v;if(Nb&&(!ba||"0"==e)||Jb)return z;"0"!=e&&(c.callback=e);I({h:b||ba,timeout:2E3,
