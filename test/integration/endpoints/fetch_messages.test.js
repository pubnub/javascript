      ...
      (status, response) => {
        ...
        if (!status.error) {
          ...
          actions = actions.sort(
            (left, right) => left.actionTimetoken - right.actionTimetoken
          );
        } else {
...
describe('fetch messages endpoints', () => {
  ...
  it('supports metadata', (done) => {
    ...
    publishMessagesToChannel(
      ...
      (messages) => {
        pubnub.fetchMessages(
          { channels: [channel], count: 25, includeMeta: true },
          (status, response) => {
            const channelMessages = response.channels[channel];

            assert.deepEqual(channelMessages[0].meta, {
      ...
      (messages) => {
        ...
        addActionsInChannel(
          ...
          (actions) => {
            setTimeout(() => {
              pubnub.fetchMessages(
                { channels: [channel], includeMessageActions: true },
                (status, response) => {
                  assert.equal(status.error, false);
                  ...
                  Object.keys(actionsByType).forEach((actionType) => {
                    Object.keys(actionsByType[actionType]).forEach(
                      (actionValue) => {
  ...
  it('should return "more" field when server sends it', (done) => {
    ...
    pubnub.fetchMessages(
      { channels: ['ch1'], includeMessageActions: true },
      (status, response) => {
        assert.equal(status.error, false);
        assert.equal(
          response.more.url,
          '/v3/history-with-actions/sub-key/s/channel/c?start=15610547826970000&max=98'
        );
  ...
  it('should request provided number of messages when count is specified for history-with-actions', (done) => {
    ...
    pubnub.fetchMessages(
      { channels: ['ch1'], includeMessageActions: true, count: 10 },
      (status, response) => {
        assert.equal(status.error, false);
        done();
      }
