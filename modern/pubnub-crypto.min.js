...
2],b[4*d+3]],e[d]=f;for(d=t;d<4*(w+1);d++){e[d]=[];for(b=0;4>b;b++)c[b]=e[d-1][b];if(0===d%t){b=c[0];f=void 0;for(f=0;4>f;f++)c[f]=c[f+1];c[3]=b;c=I(c);c[0]^=T[d/t-1]}else 6<t&&4==d%t&&(c=I(c));for(b=0;4>b;b++)e[d][b]=e[d-t][b]^c[b]}for(d=0;d<w+1;d++){p[d]=[];for(c=0;4>c;c++)p[d].push(e[4*d+c][0],e[4*d+c][1],e[4*d+c][2],e[4*d+c][3])}return p},I=function(b){for(var e=0;4>e;e++)b[e]=K[b[e]];return b},x=function(b,e){var c=[];for(i=0;i<b.length;i+=e)c[i/e]=parseInt(b.substr(i,e),16);return c},E=function(b){for(var e=
